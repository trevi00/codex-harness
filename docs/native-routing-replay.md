...
policy/corpus basis or an explicit `--evaluation-round N` creates a fresh evaluation.
Rounds are nonnegative integers, default 0; reusing a round for identical inputs is
idempotent. A round is an identity namespace, not a claim of chronological ordering.
Old runs and their blockers remain immutable. Reuse returns the retained historical result, not a fresh integrity
...
models and baseline mismatches. This follow-up now preserves fixed contract messages
for those distinctions without exposing raw OS/backend errors. Explicit reevaluation
rounds recover when the first retained evaluation was unavailable (for example, a wrong artifact root).
...
Recovery regression: the CLI first records a missing-body result, then the test
restores the exact body bytes. Default round 0 still returns the historical blocked
